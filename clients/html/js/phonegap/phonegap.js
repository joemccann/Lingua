...
PhoneGap.run_command = function() {
    if (!PhoneGap.available || !PhoneGap.queue.ready) return;
    ...
    for (var i = 1; i < args.length; i++) {
        ...
        if (typeof(arg) == 'object') dict = arg;
        else uri.push(encodeURIComponent(arg));
    }
    ...
    if (dict != null) {
        ...
        if (query_args.length > 0) url += "?" + query_args.join("&");
    }
...
Compass.prototype.watchHeading = function(successCallback, errorCallback, options) {
    ...
    var frequency = 100;
    if (typeof(options) == 'object' && options.frequency) frequency = options.frequency;
...
PhoneGap.addConstructor(function() {
    ...
    __proxyObj(navigator.geolocation, navigator._geo, ["setLocation", "getCurrentPosition", "watchPosition",
