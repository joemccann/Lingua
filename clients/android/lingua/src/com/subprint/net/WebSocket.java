...
public class WebSocket implements Runnable
{
	...
	/**
	 * Array of random characters for WebSocket key (http://trac.webkit.org/browser/trunk/WebCore/websockets/WebSocketHandshake.cpp)
	 */
	 ...
	 * 
	 * @param listener	WebSocketListener instance
	 */
	 ...
	 * 
	 * @param url		URL to connect to 
	 * @param protocol	Sub-protocol that the server must support
	 */
	...
	public void run()
	{
		if (this.readyState == ReadyState.CLOSED)
		{
			...
			try
			{
				...
				// Android 2.2 prefers IPv6, but throws "java.net.SocketException: Address family not supported by protocol"
	 ...
	 * 
	 * @param key	SelectionKey instance to connect
	 * @throws IOException
	 */
	...
	private byte[] createHandshake() throws UnsupportedEncodingException
	{
		...
		// Create the WebSocket handshake request  (http://www.whatwg.org/specs/web-socket-protocol/)
		request.append("GET ");
	...
	private void receiveHandshake()
	{
		...
		if (length >= 4 && buffer[length - 4] == WebSocket.CR && buffer[length - 3] == WebSocket.LF && buffer[length - 2] == WebSocket.CR && buffer[length - 1] == WebSocket.LF)
		{
		...
		try
		{
			...
			if (length > 1)
			{
				...
				if (handshake[0].trim().equals("HTTP/1.1 101 Web Socket Protocol Handshake"))
				{
					...
					// Iterate over the lines and check for the Upgrade and Connection fields
	...
	private void receiveFrame()
	{
		...
		else if (current == WebSocket.END_FRAME)
		{
			try
			{
				...
				{
					// Currently, only text frames are supported, so convert to string
	 ...
	 * 
	 * @param buffer	ByteBuffer instance to return length of
	 ...
	 * 
	 * @param appendTo	Buffer to be appended to
	 * @param appending	Buffer to append to the end of appendTo
	 ...
	 * 
	 * @param appendTo	Byte array to append to
	 * @param appending	Byte array to append to the end of appendTo
	 ...
	 * @see http://www.whatwg.org/specs/web-socket-protocol/
	 * @see	http://trac.webkit.org/browser/trunk/WebCore/websockets/WebSocketHandshake.cpp
	 */
	private String generateKey()
	{
		...
		do
		{
			...
			max = Integer.MAX_VALUE / spaces; // Use maximum integer, not 4294967295
		}
		while (max < -1);
